...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" exclude-result-prefixes="fo">
  ...
  <xsl:template name="LabelBase">
    ...
    <xsl:if test="$sLabelClass!='mrBannerText'">
        <xsl:choose>
    <xsl:when test="$sLabelClass='mrSingleText'">
  ...
  <xsl:template match="Table">
    ...
    <xsl:choose>
        ...
        <xsl:element name="fieldset">
          ...
          <xsl:for-each select="Row">
            <xsl:for-each select="Cell">
              <xsl:call-template name="CellImplC" />
            </xsl:for-each>
          </xsl:for-each>
        </xsl:element>
      </xsl:when>
      <xsl:when test="@UseTablesLayout = '2'">
        <!--- jquery mobile tags-->
        <xsl:for-each select="Row">
          <xsl:call-template name="RowC" />
        </xsl:for-each>
      </xsl:when>
      <xsl:when test="@UseTablesLayout = '11'">
        <!--- jquery mobile tags-->
        <xsl:element name="div">
          <xsl:attribute name="data-role">collapsibleset</xsl:attribute>
        <xsl:for-each select="Row">
          <xsl:call-template name="RowC" />
        </xsl:for-each>
        </xsl:element>
      ...
      <xsl:when test="@UseTablesLayout = '12'">
        <xsl:element name="span">
          <xsl:if test="$bIncludeCSSStyles">
            <xsl:attribute name="class">mrQuestionTable</xsl:attribute>
          </xsl:if>
  ...
  <xsl:template name="RowC">
    <xsl:choose>
      ...
      <xsl:otherwise>
          <xsl:element name="h4">
            <xsl:value-of select="Cell[1]/Label[1]/Text"/>
          </xsl:element>
        <xsl:element name="fieldset">
          <xsl:attribute name="data-role">controlgroup</xsl:attribute>
          <xsl:attribute name="data-type">horizontal</xsl:attribute>
            <xsl:for-each select="Cell">
              <xsl:call-template name="CellImplD" />
            </xsl:for-each>
        </xsl:element>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template name="RowT">
    <xsl:choose>
      <xsl:when test="@IsHeading">
        <xsl:element name="thead">
          <xsl:element name="tr">
            <xsl:for-each select="Cell">
              <xsl:call-template name="CellT"/>
            </xsl:for-each>
          </xsl:element>
        </xsl:element>
      </xsl:when>
